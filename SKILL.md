...
description: |
  ...
  driving a local XLA workflow with Copilot. Use when the real goal is lowering
...
This skill turns Copilot into a whole-model latency optimizer for the local MUSA
...
The execution environment is the current local machine/session.

Required inputs:

- local workspace root
- actual repository roots inside that workspace
...
- keep `AE_USE_COMMON_RUNNER=1` and point `AE_COMMON_RUNNER_PATH` to
  `scripts/common_runner.sh` for whole-model runs

Use `scripts/local_xla_exec.sh` as the local launcher for commands.
Default mode is `local`.

## Non-Negotiable Goal

The only success metric is **whole-model latency reduction** in the current
local environment.
...
- For this repository, prefer `model/meta_graph_2` and execute
  `scripts/common_runner.sh` as the benchmark entry
- Treat `result_graph2.md` `w_musa_xla` at `bsz=1024` (`457.706 ms`) as the baseline
- Use `batch_size=1024` as the default validation setting unless explicitly overridden
  by the user in session
- Record:
  - environment
  - benchmark command
  - target latency
  - measured `avg_latency_ms` from `outputs/*/common_runner.log`
  - correctness result
...
Default reusable entries in `templates/task.yaml` now point to:

- `scripts/run_musa_rewriter_test.sh`
- `scripts/run_musa_custom_call_test.sh`

Use env overrides when needed:

- `AE_REWRITER_TEST_TARGET`
- `AE_CUSTOM_CALL_TEST_TARGET`
- `AE_WITH_RUNTIME_REG_TEST`
...
After each code change and each `common_runner.sh` run, enter the newly created
experiment output directory and write `experiment_log.md` summarizing:

- optimization idea
- core code changes
- the measured `avg_latency_ms`
- comparison against baseline

For operator loop bootstrap, set:

- `AE_SEMANTIC_OP_ID`
- `AE_OPERATOR_SEED_PATH`
- optional `AE_PTX_SEMANTICS`

Then generate a reusable task file:

    scripts/operator_task_bootstrap.sh artifacts/operator_task.local.yaml
...
For a new task, the minimum useful sequence is:
    ...
    export AE_ENV_FILE=./config/local.env
    # keep model_target in scripts/common_runner.sh aligned with model_name in templates/task.yaml
