...
if [[ -n "$BUILD_ENTRY" ]]; then
  echo "=== BUILD CHECK ==="
  "$ROOT_DIR/scripts/remote_xla_exec.sh" "$BUILD_ENTRY"
fi

if [[ -n "$REWRITER_TEST_ENTRY" ]]; then
  echo "=== REWRITER TEST ==="
  "$ROOT_DIR/scripts/remote_xla_exec.sh" "$REWRITER_TEST_ENTRY"
fi

if [[ -n "$CUSTOM_CALL_TEST_ENTRY" ]]; then
  echo "=== CUSTOM CALL TEST ==="
  "$ROOT_DIR/scripts/remote_xla_exec.sh" "$CUSTOM_CALL_TEST_ENTRY"
fi
