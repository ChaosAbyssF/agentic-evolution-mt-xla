...
case "$MODE" in
  targeted)
    ...
    "$ROOT_DIR/scripts/local_xla_exec.sh" "$(build_profile_entry "$TARGETED_PREFIX")" | tee "$OUT_DIR/msys_targeted.log"
    ;;
  full)
    [[ -n "$FULL_PREFIX" ]] || { echo "operator_profile_full_report_prefix is required" >&2; exit 1; }
    "$ROOT_DIR/scripts/local_xla_exec.sh" "$(build_profile_entry "$FULL_PREFIX")" | tee "$OUT_DIR/msys_full.log"
    ;;
  *)
