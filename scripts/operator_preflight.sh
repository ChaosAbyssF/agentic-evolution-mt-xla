...
if [[ -z "$SEMANTIC_OP_ID" || "$SEMANTIC_OP_ID" == "fill-me" || "$SEMANTIC_OP_ID" == from* ]]; then
  SEMANTIC_OP_ID=${AE_SEMANTIC_OP_ID:-}
fi
if [[ -z "$SEED_PATH" || "$SEED_PATH" == "fill-me" || "$SEED_PATH" == from* ]]; then
  SEED_PATH=${AE_OPERATOR_SEED_PATH:-}
fi
if [[ -z "$PTX_SEMANTICS" || "$PTX_SEMANTICS" == "fill-me" || "$PTX_SEMANTICS" == from* ]]; then
  PTX_SEMANTICS=${AE_PTX_SEMANTICS:-}
fi
if [[ -z "$BASELINE_VALUE" || "$BASELINE_VALUE" == "fill-me" ]]; then
  BASELINE_VALUE=${AE_OPERATOR_BASELINE_MS:-}
fi
