...
- `SKILL.md`: Copilot-facing behavior contract

### `config/`

User-owned environment configuration templates. These files define workspace
values without hardcoding them into the project. The current default is local
in-container execution via `AE_LOCAL_WORKDIR`.
